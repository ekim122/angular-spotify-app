...
export class MusicDataService {
  ...
  getAlbumsByArtistId(id:any):Observable<SpotifyApi.ArtistsAlbumsResponse>{
    let params = new HttpParams().set('include_groups', 'album,single').set('limit', '50');
    return this.spotifyToken.getBearerToken().pipe(
      mergeMap(token=>{
        return this.http.get<SpotifyApi.ArtistsAlbumsResponse>(`https://api.spotify.com/v1/artists/${id}/albums`, {
  ...
  searchArtists(searchString:any): Observable<SpotifyApi.ArtistSearchResponse>{
    let params = new HttpParams().set('q', searchString).set('type', 'artist').set('limit', '50')
    return this.spotifyToken.getBearerToken().pipe(
      mergeMap(token=>{
        return this.http.get<SpotifyApi.ArtistSearchResponse>(`https://api.spotify.com/v1/search`, {
          params, headers: {"Authorization":`Bearer ${token}`}
        });
      })
    );
